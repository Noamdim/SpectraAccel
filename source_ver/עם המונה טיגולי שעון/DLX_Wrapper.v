...
module DLX_Wrapper (
    ...
    //wire raw_IN_INIT;
    //wire accel_active = debug_accel[3];
    //assign IN_INIT = raw_IN_INIT & ~accel_active;

    DLX_MUSER_Top_Level u_dlx_core (
        .ACK_N(ACK_N),
        .CLK(CLK),
        .DI(DI),
        .D_addr(D_addr),
    ...
    // 10-bit Counter for clocks passed since the last classify_pulse and 1-bit overflow
    reg [13:0] clks_since_classify;
    reg       clks_since_classify_ovf;
    
    always @(posedge CLK or posedge RESET) begin
        if (RESET) begin
            clks_since_classify <= 14'd0;
            clks_since_classify_ovf <= 1'b0;
        end else if (classify_pulse) begin
            clks_since_classify <= 14'd0;
            clks_since_classify_ovf <= 1'b0;
        end else begin
            if (clks_since_classify == 14'h3FFF) begin
                clks_since_classify_ovf <= 1'b1; // Stop counting and set overflow when max value is reached
            end else begin
                clks_since_classify <= clks_since_classify + 1;
            end
        end
    end
    ...
    // Inject the counter into bits 30:21 and overflow into bit 31
    assign LA_DATA_IN[30:17] = clks_since_classify;
    assign LA_DATA_IN[31]    = clks_since_classify_ovf;

...
def files_to_cube(band_paths, window=None):
    """
    Reads Sentinel-2 JP2 bands into a 3D numpy cube.

    Parameters
    ----------
    band_paths : dict
        Mapping of band name to file path.
    window : tuple, optional
        (row_start, row_end, col_start, col_end) in 10m pixel coordinates.
        If provided, only the sub-region is decoded from disk — much faster
        for large tiles. Combine all your post-load slices into one window.
        Example: cube[6400:8000, 2500:4000][430:850, 0:400]
                 -> window=(6830, 7250, 2500, 2900)
    """
    print("#################################################################################################################")
    ...
    # --- Build rasterio Window for 10m bands ---
    if window is not None:
        r0, r1, c0, c1 = window
        h, w = r1 - r0, c1 - c0
        win_10m = rasterio.windows.Window(col_off=c0, row_off=r0, width=w, height=h)

    for b in ["B02", "B03", "B04", "B08"]:
        with rasterio.open(band_paths[b]) as src:
            if window is not None:
                bands_10m[b] = src.read(1, window=win_10m).astype(np.float32)
            else:
    ...
    for b in ["B05", "B06", "B07", "B11", "B12"]:
        with rasterio.open(band_paths[b]) as src:
            if window is not None:
                # 20m pixels are 2x larger — scale the window to 20m grid.
                # Use ceiling division so we read exactly the right number of 20m pixels:
                # (h+1)//2 == ceil(h/2). This ensures the resampled output is true 10m,
                # keeping 20m bands spatially aligned with the 10m bands.
                win_20m = rasterio.windows.Window(
                    col_off=c0 // 2, row_off=r0 // 2,
                    width=(w + 1) // 2, height=(h + 1) // 2
                )
                # Read only the 20m sub-region and immediately upsample to 10m size
                data_20m = src.read(
                    1, window=win_20m,
                    out_shape=(h, w),
...
def sentinel_data_processor(cube, thresholds, controllers, classifier_rules, rgb_library):
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            # Pass the normalized uint8 vector directly
            pixel = cube_uint8[r, c, :]
            pixel_color = process_pixel(pixel, thresholds, controllers, classifier_rules, rgb_values_list)
            final_img[r, c] = pixel_color

    end_time = time.perf_counter()
    classification_time = end_time - start_time
    print(f"Classification run in {classification_time:.6f} seconds")
    ...
    return fig, classification_time
...
############################# MAIN ######################################
#########################################################################
path_pre = "C:\\Users\\nooam\\.gemini\\antigravity-ide\\scratch\\hardwareSIM\\photosData\\"
...
band_paths_isr_leb_zomet_golani = {
...
cubes = {
    "zomet": files_to_cube(band_paths_isr_leb_zomet_golani, window=(6830, 7250, 2500, 2900)),
    "ams": files_to_cube(band_paths_ams, window=(3000, 3500, 1500, 2000)),
    "nat": files_to_cube(band_paths_nat, window=(5550, 5900, 7450, 8500))
}
cube = cubes["zomet"]
            ...
            children=[
                ...
                dcc.Tab(label="Spectral Classification", value="tab-classification", children=[
                    ...
                    dcc.RadioItems(
                        id="image-selector",
                        options=[
                            {"label": " Zomet Golani", "value": "zomet"},
                            {"label": " Amsterdam", "value": "ams"},
                            {"label": " Yahud Natbag", "value": "nat"}
                        ],
                        value="zomet",
                        inline=True,
...
def run_classification(n_clicks, selected_image, *args):
    ...
    # Run classification
    active_cube = cubes[selected_image]
    new_fig, classification_time = sentinel_data_processor(active_cube, new_thresholds, new_controllers, new_rules, new_rgb_library)
    
    return new_fig, f"Classification complete! ({time.strftime('%H:%M:%S')}) | Hardware Time: {classification_time:.4f}s"
